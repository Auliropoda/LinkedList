...
class LinkedList
{
	...
	struct Node
	{
		Node(const ValueType& value, Node* next = nullptr); //сделано
		~Node(); //сделано
...
public:
	////
	LinkedList();  //сделано
	LinkedList(const LinkedList& copyList);  //сделано
	LinkedList& operator=(const LinkedList& copyList); //сделано

	LinkedList(LinkedList&& moveList) noexcept; //сделано
	LinkedList& operator=(LinkedList&& moveList) noexcept; //сделано
	...
	LinkedList::Node* getNode(const size_t pos) const; //сделано
